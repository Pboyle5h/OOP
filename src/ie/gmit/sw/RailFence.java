...
public class RailFence {
	...
	public String encrypt(String cypherText, int key) {
		...
		char[][] matrix = new char[key][cypherText.length()]; // The array is
																// filled with
																// chars with
																// initial
																// values of
		...
		for (int i = 0; i < cypherText.length(); i++) { // Loop over the
														// plaintext
			matrix[row][i] = cypherText.charAt(i); // Add the next character in
		...
		StringBuffer sb = new StringBuffer(); // A string buffer allows a string
												// to be built efficiently
		for (row = 0; row < matrix.length; row++) { // Loop over each row in the
													// matrix
			for (int col = 0; col < matrix[row].length; col++) { // Loop over
																	// each
																	// column in
																	// the
																	// matrix
				if (matrix[row][col] > '0')
					sb.append(matrix[row][col]); // Extract the character at the
													// row/col position if it's
													// not 0.
			}
		}

		return sb.toString(); // Convert the StringBuffer into a String and
								// return it
	}
	...
	public String decrypt(String cypherText, int key) {
		...
		do {
			...
			for (int i = 0; i < cypherText.length(); i++) { // Loop over the
															// plaintext
				if (row == targetRow) {
					matrix[row][i] = cypherText.charAt(index); // Add the next
																// character in
																// the plaintext
																// to the array
					index++;
	...
	private void printMatrix(char[][] matrix) {
		for (int row = 0; row < matrix.length; row++) { // Loop over each row in
														// the matrix
			for (int col = 0; col < matrix[row].length; col++) { // Loop over
																	// each
																	// column in
																	// the
																	// matrix
				System.out.print(matrix[row][col]); // Output the value at
													// row/column index
